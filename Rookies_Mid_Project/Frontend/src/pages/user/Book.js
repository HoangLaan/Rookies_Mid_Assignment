...
import { getListBook, postRequest } from "../../services/Service";
...
function Book() {
    ...
    const BorrowBook = (id) => {
        if (borrowedBook.includes(id)) {
            setBorrowedBook(borrowedBook.filter(bookId => bookId !== id));
        } else {
            if (borrowedBook.length >= 5) {
                window.alert("You can only borrow 5 books at the same time")
            } else {
                setBorrowedBook([...borrowedBook, id])
            }
        }
    }
    ...
    const handleBorrowRequest = async (userId, books) => {
        var borrowRequest = { UserId: userId, BookIds: books }
        try {
            await postRequest(borrowRequest)
            window.alert('Your request had been sent!')
            setBorrowedBook([])
        } catch (error) {
            console.log("Borrow Error", error)
            window.alert('You have limit 3 requests per month!')
        }
    }
    ...
    return (
        <div className="container mx-auto px-4 ">
            ...
            {loading && <img className="opacity-40" src={loadingg} alt="Loading" />}
            ...
            <div className="border border-gray-500 rounded p-4 my-36">
                    ...
                    return (
                        <div key={book.id}>
                            <table className="min-w-full bg-white">
                                <tbody>
                                    <tr>
                                        <td className="py-2 px-4 border-b border-gray-200 text-center w-1/12">{id + 1}</td>
                                        <td className="py-2 px-4 text-center border-b border-gray-200 w-1/12">{book.title}</td>
                                        <td className="py-2 text-center px-4 border-b border-gray-200 w-1/12">{book.author}</td>
                                        <td className="py-2 text-center px-4 border-b border-gray-200 w-2/12">
                                            {book.description.length > 40 ? `${book.description.substring(0, 40)}...` : book.description}
                                        </td>
                                        <td className="py-2 text-center px-4 border-b border-gray-200 w-1/12">
                                            {book.quantity}
                                        </td>
                                        <td className="py-2 text-center px-4 border-b border-gray-200 w-1/12">
                                            {book.categoryNames.length > 10 ? `${book.categoryNames.substring(0, 10)}...` : book.categoryNames}
                                        </td>
                                        <td className="py-2 px-4 border-b border-gray-200 w-2/12">
                                            <div className="flex justify-center">
                                                <button
                                                    onClick={() => BorrowBook(book.id)}
                                                    className={`w-24 font-bold py-1 px-2 rounded mr-2 ${borrowedBook.includes(book.id)
                                                        ? "bg-yellow-500 hover:bg-yellow-700 text-white"
                                                        : "bg-blue-500 hover:bg-blue-700 text-white"
                                                        }`}
                                                >
                                                    {borrowedBook.includes(book.id) ? "Back" : "Borrow"}
                                                </button>
                                                <button onClick={() => DetailBook(book.id)} className="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-2 rounded">Detail</button>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
                ...
                <div className="w-full flex justify-end pt-5">
                    <span className="ml-2 text-lg font-semibold text-blue-500 border border-gray-300 rounded-full px-2 py-1 bg-white mr-4">
                        {borrowedBook.length}
                    </span>
                    <button
                        className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                        onClick={() => handleBorrowRequest(user.id, borrowedBook)}
