...
import { approveRequest, getRequest, rejectRequest } from "../../services/Service";
                ...
                {!loading &&
                    <div>
                        ...
                        <table className="min-w-full bg-white">
                            <thead className=''>
                                <tr>
                                    <th className="py-4 px-4 border-b border-gray-200 text-center w-2/12">ID</th>
                                    <th className="py-2 px-4 border-b border-gray-200 text-center w-2/12">Username</th>
                                    <th className="py-2 px-4 border-b border-gray-200 text-center w-2/12">Book</th>
                    ...
                    return (
                        <div key={id} >
                            <table className="min-w-full bg-white" >
                                <tbody>
                                    <tr>
                                        <td className="py-2 px-4 text-center border-b border-gray-200 w-2/12 ">{request.id}</td>
                                        <td className="py-2 px-4 text-center border-b border-gray-200 w-2/12">{request.userName}</td>
                                        <td className="py-2 px-4 text-center border-b border-gray-200 w-2/12">{request.title}</td>
                                        ...
                                        <td className="py-2 px-4 text-center border-b border-gray-200 w-4/12">
                                            {request.requestStatus === 0 ? (
                                                <div>
                                                    <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-2 rounded mr-2" onClick={() => handleApprove(request.id)}>Approve</button>
                                                    <button className="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded" onClick={() => handleReject(request.id)}>Reject</button>
                                                </div>
                                            ) : (
